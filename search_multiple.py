...
from sdr_compare import SDR_Compare
import numpy
import argparse
import os
import json
...
def build_sdr_model(result_dic, query_dic):
    corpus = []
    pids = []
    for key in result_dic:
        pids.append(key)
        ...
        for line in tqdm(eval):
            ...
            for actual_id in actual_ids:
                ...
                if actual_id in collection_dic.keys():
                    query_tokens.extend(collection_dic[actual_id])
            result_dict = {}
        ...
        for line_index, line in tqdm(enumerate(eval)):
            ...
            for id in run_id_list:
                ...
                if id in collection_dic.keys():
                    ...
                    print(id, "run id not found")
            pids, model, overall_corpus = build_sdr_full_model(result_dict, ' '.join(query_tokens),
                                                               query_dictionary)
            if type == "original":
                scores = model.get_scores_multiple(query_tokens, overall_corpus,weight_dict, sys_id, storing_dir)
            else:
                scores = model.get_scores_multiple(query_dictionary, overall_corpus,weight_dict, sys_id, storing_dir)
            indices = sorted(range(len(scores)), key=lambda k: scores[k])[::-1]
            pid_sorted = [str(pids[i]) for i in indices]
            score_values = [scores[i] for i in indices]
            order_index = 1
...
if __name__ == "__main__":
    ...
    parser.add_argument("--DATA_DIR", type=str, default="2018/multiple/")
    parser.add_argument("--METHOD", type=str, required=True)
    parser.add_argument("--format", type=str, default='TEST')
